...
public class OrgnizationController : ControllerBase
{
    ...
    //[HttpGet("GetAllOrgs")]
    //public async Task<ActionResult<List<OrganizationDto>>> GetAllOrgs()
    //{
    ...
    public async Task<ActionResult> SearchOrganizations(
    [FromQuery] string? search,
    [FromQuery] string? startsWith,
    [FromQuery] string? sortBy = "OrgId",
    ...
    [FromQuery] int pageSize = 10)
    {
        ...
        // 🔍 Search by OrgId or OrgName
        if (!string.IsNullOrEmpty(search))
        {
            query = query.Where(o =>
                o.OrgId.Contains(search) ||
                o.OrgName.Contains(search));
        }

        // 🔎 Filter by OrgId prefix
        if (!string.IsNullOrEmpty(startsWith))
        {
            query = query.Where(o => o.OrgId.StartsWith(startsWith));
        }

        // ↕️ Sorting
        query = (sortBy.ToLower(), sortOrder.ToLower()) switch
        {
            ("orgname", "desc") => query.OrderByDescending(o => o.OrgName),
            ("orgname", _) => query.OrderBy(o => o.OrgName),

            ("orgid", "desc") => query.OrderByDescending(o => o.OrgId),
            _ => query.OrderBy(o => o.OrgId),
        };
    ...
    //[HttpPost]
    //public async Task<ActionResult<Organization>> Post(Organization org)
    //{
    //    _context.Organizations.Add(org);
    //    await _context.SaveChangesAsync();
    //    return CreatedAtAction(nameof(Get), new { id = org.OrgId }, org);
    //}

    [HttpPost]
    public async Task<ActionResult<Organization>> Post(Organization org)
    {
        var parts = org.OrgId.Split('.');
        int calculatedLevel = parts.Length;

        // Validate Level
        if (org.LvlNo != calculatedLevel)
        {
            return BadRequest($"Invalid Level. Orgnization {org.OrgId} should have Level {calculatedLevel}");
        }
        ...
        if (calculatedLevel > 1)
        {
            ...
            bool parentExists = await _context.Organizations
                .AnyAsync(o => o.OrgId == parentCode);

            if (!parentExists)
            {
                return BadRequest($"Parent organization {parentCode} must exist before creating {org.OrgId}");
            }
        }
    ...
    [HttpPost("CreateOrg")]
    public async Task<ActionResult<Organization>> CreateOrg(Organization org)
    {
        // Split OrgId into segments
        var segments = org.OrgId.Split('.');
        int levelCount = segments.Length;
        ...
        {
            string currentCode = string.Join(".", segments.Take(i));

            // Get OrgName for this level
            var parentOrg = await _context.Organizations
                .Where(o => o.OrgId == currentCode)
                .Select(o => o.OrgName)
                .FirstOrDefaultAsync();

            string orgNameToSet = i == levelCount ? org.OrgName : parentOrg;
            string segIdToSet = segments[i - 1]; // Last segment for this level

            switch (i)
            {
                case 1:
                    org.L1OrgName = orgNameToSet;
                    org.L1OrgSegId = segIdToSet;
                    break;
                case 2:
                    org.L2OrgName = orgNameToSet;
                    org.L2OrgSegId = segIdToSet;
                    break;
                case 3:
                    org.L3OrgName = orgNameToSet;
                    org.L3OrgSegId = segIdToSet;
                    break;
                case 4:
                    org.L4OrgName = orgNameToSet;
                    org.L4OrgSegId = segIdToSet;
                    break;
                case 5:
                    org.L5OrgName = orgNameToSet;
                    org.L5OrgSegId = segIdToSet;
                    break;
                case 6:
                    org.L6OrgName = orgNameToSet;
                    org.L6OrgSegId = segIdToSet;
                    break;
                case 7:
                    org.L7OrgName = orgNameToSet;
                    org.L7OrgSegId = segIdToSet;
                    break;
                case 8:
                    org.L8OrgName = orgNameToSet;
                    org.L8OrgSegId = segIdToSet;
                    break;
                case 9:
                    org.L9OrgName = orgNameToSet;
                    org.L9OrgSegId = segIdToSet;
                    break;
                case 10:
                    org.L10OrgSegId = segIdToSet;
                    break;
            }
        }
    ...
    {
        if (id != org.OrgId) return BadRequest();
