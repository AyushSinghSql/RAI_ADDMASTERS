...
namespace PlanningAPI.Controllers
{
    ...
    public class SubPeriodController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create(SubPeriodDto dto)
        {
            var companyId = string.IsNullOrEmpty(dto.CompanyId) ? "1" : dto.CompanyId;
            var exists = await _context.SubPeriods.AnyAsync(x =>
                x.FyCd == dto.FyCd &&
                x.PeriodNo == dto.PeriodNo &&
                x.SubPeriodNo == dto.SubPeriodNo &&
        ...
        public async Task<IActionResult> Update(string fyCd, int periodNo, int subPeriodNo, SubPeriodDto dto)
        {
            var companyId = string.IsNullOrEmpty(dto.CompanyId) ? "1" : dto.CompanyId;
            var entity = await _context.SubPeriods
                .FirstOrDefaultAsync(x => x.FyCd == fyCd && x.PeriodNo == periodNo && x.SubPeriodNo == subPeriodNo && (x.CompanyId == companyId || string.IsNullOrEmpty(x.CompanyId)));

            if (entity == null)
            {
                entity = await _context.SubPeriods
                    .FirstOrDefaultAsync(x => x.FyCd == fyCd && x.PeriodNo == periodNo && x.SubPeriodNo == subPeriodNo);
            }
        ...
        // ? DELETE
        [HttpDelete("{fyCd}/{periodNo}/{subPeriodNo}")]
        public async Task<IActionResult> Delete(string fyCd, int periodNo, int subPeriodNo, [FromQuery] string? companyId = null)
        {
            var entity = await _context.SubPeriods
                .FirstOrDefaultAsync(x => x.FyCd == fyCd && x.PeriodNo == periodNo && x.SubPeriodNo == subPeriodNo && (string.IsNullOrEmpty(companyId) || x.CompanyId == companyId));
